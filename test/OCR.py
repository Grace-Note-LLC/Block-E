...
class VideoStream:
    ...
    def __init__(self, src=0):
        ...
        self.stream.set(cv2.CAP_PROP_FRAME_WIDTH, 1000)
        self.stream.set(cv2.CAP_PROP_FRAME_HEIGHT, 1000)
    ...
    def ocr(self):
        ...
        while not self.stopped:
            if self.exchange is not None:  # Defends against an undefined VideoStream reference
                ...
                self.boxes = pytesseract.image_to_data(frame)
...
def ocr_stream(crop: list[int, int], source: int = 0, view_mode: int = 1):
    """
